...
> 当前仓库已包含 Windows 11 和 macOS 适配代码。macOS 用户优先使用 Releases 中的 `.dmg` 安装包；从源码运行仍需要 Node.js 和 Claude Code。
...
- **回复气泡摘要** — Claude 开始回复后，Clawd 会在头顶显示最近 1 到 2 行摘要，并在流式回复过程中持续刷新
...
当 Claude 开始输出回复时，Clawd 会在头顶弹出一个小气泡，显示最近回复内容的 1 到 2 行摘要。只要 Claude 还在流式输出，气泡就会继续更新；回复结束后，气泡会自动收起。
...
如果你是 macOS 用户，最简单的方式是直接安装发布包：

1. 打开 [Releases 页面](https://github.com/rullerzhou-afk/clawd-on-desk/releases/latest)
2. Apple 芯片（M1/M2/M3/M4）下载 `Clawd.on.Desk-*-arm64.dmg`
3. Intel Mac 下载 `Clawd.on.Desk-*.dmg`
4. 将 `Clawd on Desk.app` 拖到“应用程序”
5. 首次打开如果被系统拦截，右键应用 →“打开”，或到“系统设置”→“隐私与安全性”里允许运行

首次启动后，Clawd 会尝试自动注册 Claude Code hooks。若你使用源码运行，也可以手动执行 `node hooks/install.js`。
...
src/
  ...
  preload-speech.js # 回复气泡窗口的 IPC 桥接
  index.html     # 页面结构
  speech.html    # Claude 回复摘要气泡界面
hooks/
...
- 这个仓库适合直接作为 GitHub 开源项目发布。
- 你当前维护的版本重点是两件事：`macOS 适配` 和 `Claude 回复气泡`。
- 如果你准备以自己的仓库名发布，记得同步修改 `package.json` 里的 GitHub 发布地址，以及 README 里的 clone / Releases 链接。
