...
const CLAWD_PORT = 23334;
const CLAUDE_PROJECTS_DIR = path.join(os.homedir(), ".claude", "projects");
const SPEECH_BUBBLE_WIDTH = 320;
const SPEECH_BUBBLE_HEIGHT = 140;
const SPEECH_HIDE_DELAY_MS = 12000;
const SPEECH_END_HIDE_DELAY_MS = 5000;
const SPEECH_RESOLVE_RETRY_MS = 500;
const SPEECH_RESOLVE_MAX_ATTEMPTS = 20;
const SPEECH_MAX_CHARS = 220;
const SPEECH_SUMMARY_MAX_LINES = 2;
const SPEECH_SUMMARY_LINE_CHARS = 22;
...
const i18n = {
  en: {
    ...
    screenshot: "Screenshot & Ask Yuanbao",
    screenshotSending: "Sending to Yuanbao...",
    screenshotSent: "Sent! Check Yuanbao for reply",
    quit: "Quit",
  },
  zh: {
    size: "大小",
    ...
    sessionHrAgo: "{n}小时前",
    screenshot: "截图提问（元宝）",
    screenshotSending: "正在发到元宝...",
    screenshotSent: "已发送！请查看元宝回复",
    quit: "退出",
  },
};
...
function detectDefaultLanguage() {
  ...
  return locales.some((locale) => /^zh(?:-|_|$)/i.test(String(locale))) ? "zh" : "en";
}
...
let speechCurrentSessionId = null;
let speechCurrentText = "";
const speechTrackers = new Map(); // sessionId → { transcriptPath, watcher, resolveTimer, refreshTimer, resolveAttempts, lastText }
...
function updateSession(sessionId, state, event, sourcePid, cwd) {
  handleSpeechEvent(sessionId, event);
...
function getSpeechTracker(sessionId) {
  if (!speechTrackers.has(sessionId)) {
    speechTrackers.set(sessionId, {
      transcriptPath: null,
      watcher: null,
      resolveTimer: null,
      refreshTimer: null,
  ...
  return speechTrackers.get(sessionId);
}
...
function scheduleSpeechHide(delayMs) {
...
function cleanupSpeechTracker(sessionId) {
  const tracker = speechTrackers.get(sessionId);
  if (!tracker) return;
  if (tracker.watcher) tracker.watcher.close();
  if (tracker.resolveTimer) clearTimeout(tracker.resolveTimer);
  if (tracker.refreshTimer) clearTimeout(tracker.refreshTimer);
  speechTrackers.delete(sessionId);
}
...
function findTranscriptPath(sessionId) {
  try {
    ...
    for (const dirent of projectDirs) {
      ...
      const candidate = path.join(CLAUDE_PROJECTS_DIR, dirent.name, `${sessionId}.jsonl`);
...
function normalizeSpeechText(text) {
  ...
  return normalized.length > SPEECH_MAX_CHARS
    ? `${normalized.slice(0, SPEECH_MAX_CHARS - 1).trimEnd()}…`
    : normalized;
}

function cleanSpeechSummaryLine(line) {
  return String(line || "")
    .replace(/!\[[^\]]*]\([^)]*\)/g, "")
    .replace(/\[([^\]]+)\]\([^)]*\)/g, "$1")
    .replace(/`([^`]+)`/g, "$1")
    .replace(/^\s{0,3}(?:[-*+]\s+|\d+[.)]\s+|>\s*|#+\s*)/, "")
    .replace(/\s+/g, " ")
    .trim();
}

function truncateSpeechSummaryLine(text, maxChars = SPEECH_SUMMARY_LINE_CHARS) {
  const chars = Array.from(String(text || "").trim());
  if (chars.length <= maxChars) return chars.join("");
  return `${chars.slice(0, maxChars - 1).join("").trimEnd()}…`;
}

function splitSpeechSummaryLine(text) {
  const chars = Array.from(String(text || "").trim());
  if (!chars.length) return [];
  if (chars.length <= SPEECH_SUMMARY_LINE_CHARS) return [chars.join("")];
  return [
    truncateSpeechSummaryLine(chars.slice(0, SPEECH_SUMMARY_LINE_CHARS).join(""), SPEECH_SUMMARY_LINE_CHARS),
    truncateSpeechSummaryLine(chars.slice(SPEECH_SUMMARY_LINE_CHARS).join(""), SPEECH_SUMMARY_LINE_CHARS),
  ].filter(Boolean);
}

function extractSpeechSummary(text) {
  ...
  for (const candidate of candidates) {
    const line = truncateSpeechSummaryLine(candidate);
    if (!line) continue;
    summaryLines.push(line);
    if (summaryLines.length >= SPEECH_SUMMARY_MAX_LINES) break;
  }

  if (summaryLines.length === 0) {
    return splitSpeechSummaryLine(rawLines[0]).slice(0, SPEECH_SUMMARY_MAX_LINES).join("\n");
  }

  return summaryLines.slice(0, SPEECH_SUMMARY_MAX_LINES).join("\n");
}

function parseLatestAssistantText(transcriptPath) {
  try {
    ...
    for (let i = startIndex; i < entries.length; i++) {
      ...
      latest = {
        text,
        summary: extractSpeechSummary(text),
        isFinal: entry.message.stop_reason != null,
      };
    }
    return latest;
  } catch {
...
function getSpeechBubbleBounds() {
  ...
  const x = Math.max(wa.x + 8, Math.min(bounds.x + bounds.width - 28, wa.x + wa.width - SPEECH_BUBBLE_WIDTH - 8));
  const y = Math.max(wa.y + 8, bounds.y - SPEECH_BUBBLE_HEIGHT + 18);
  return { x, y, width: SPEECH_BUBBLE_WIDTH, height: SPEECH_BUBBLE_HEIGHT };
}

function updateSpeechBubblePosition() {
  if (!speechBubble || speechBubble.isDestroyed() || !speechBubble.isVisible()) return;
  const bounds = getSpeechBubbleBounds();
  if (bounds) speechBubble.setBounds(bounds);
}

function ensureSpeechBubble() {
  if (speechBubble && !speechBubble.isDestroyed()) return speechBubble;
  const bounds = getSpeechBubbleBounds();
  if (!bounds) return null;

  speechBubble = new BrowserWindow({
    x: bounds.x,
    y: bounds.y,
    width: bounds.width,
  ...
  speechBubble.setIgnoreMouseEvents(true);
  if (isMac) {
    speechBubble.setVisibleOnAllWorkspaces(true, { visibleOnFullScreen: false });
    speechBubble.setAlwaysOnTop(true, "floating");
  } else {
    speechBubble.setAlwaysOnTop(true, WIN_TOPMOST_LEVEL);
  }

  speechBubble.loadFile(path.join(__dirname, "speech.html"));
  speechBubble.on("closed", () => {
    speechBubble = null;
  });
  guardAlwaysOnTop(speechBubble);
  return speechBubble;
}

function showSpeechBubble(text) {
  if (doNotDisturb || !text) return;
  const bubbleWindow = ensureSpeechBubble();
  if (!bubbleWindow) return;
  speechCurrentText = text;
  updateSpeechBubblePosition();
  const send = () => {
    if (!speechBubble || speechBubble.isDestroyed()) return;
    speechBubble.webContents.send("speech-update", { text: speechCurrentText, lang });
    speechBubble.showInactive();
  };
...
function hideSpeechBubble(immediate = false) {
  speechCurrentText = "";
  if (speechBubble && !speechBubble.isDestroyed()) {
    speechBubble.webContents.send("speech-hide");
    if (immediate) {
      speechBubble.hide();
      return;
    }
    setTimeout(() => {
      if (speechBubble && !speechBubble.isDestroyed()) speechBubble.hide();
    }, 180);
  }
}

function refreshSpeechText(sessionId) {
  const tracker = getSpeechTracker(sessionId);
  if (!tracker.transcriptPath) return;
  if (tracker.resetAt) {
    try {
      const stat = fs.statSync(tracker.transcriptPath);
      if (stat.mtimeMs < tracker.resetAt) return;
    } catch {
      return;
    }
    tracker.resetAt = 0;
  }
  const latest = parseLatestAssistantText(tracker.transcriptPath);
  const shouldHideAfterFinal = latest?.isFinal && speechCurrentSessionId === sessionId;
  const summary = latest?.summary || "";
  if (!summary || summary === tracker.lastText) {
    if (shouldHideAfterFinal) scheduleSpeechHide(SPEECH_END_HIDE_DELAY_MS);
    return;
  }
  tracker.lastText = summary;
  if (speechCurrentSessionId !== sessionId) return;
  clearSpeechHideTimer();
  showSpeechBubble(summary);
  if (shouldHideAfterFinal) scheduleSpeechHide(SPEECH_END_HIDE_DELAY_MS);
}

function queueSpeechRefresh(sessionId, delayMs = 80) {
  const tracker = getSpeechTracker(sessionId);
  if (tracker.refreshTimer) clearTimeout(tracker.refreshTimer);
  tracker.refreshTimer = setTimeout(() => {
    tracker.refreshTimer = null;
    refreshSpeechText(sessionId);
  }, delayMs);
}

function watchTranscript(sessionId, transcriptPath) {
  const tracker = getSpeechTracker(sessionId);
  if (tracker.watcher || !transcriptPath) return;
  tracker.transcriptPath = transcriptPath;
  refreshSpeechText(sessionId);
  try {
    tracker.watcher = fs.watch(transcriptPath, { persistent: false }, () => {
      queueSpeechRefresh(sessionId);
    });
...
function resolveAndWatchTranscript(sessionId) {
  const tracker = getSpeechTracker(sessionId);
  const transcriptPath = findTranscriptPath(sessionId);
  if (transcriptPath) {
    tracker.resolveAttempts = 0;
    if (tracker.resolveTimer) { clearTimeout(tracker.resolveTimer); tracker.resolveTimer = null; }
    watchTranscript(sessionId, transcriptPath);
    return;
  }
  if (tracker.resolveAttempts >= SPEECH_RESOLVE_MAX_ATTEMPTS) return;
  tracker.resolveAttempts += 1;
  if (tracker.resolveTimer) clearTimeout(tracker.resolveTimer);
  tracker.resolveTimer = setTimeout(() => {
    tracker.resolveTimer = null;
    resolveAndWatchTranscript(sessionId);
  }, SPEECH_RESOLVE_RETRY_MS);
}

function activateSpeechTracking(sessionId, options = {}) {
  const { reset = false } = options;
  speechCurrentSessionId = sessionId;
  clearSpeechHideTimer();
  const tracker = getSpeechTracker(sessionId);
  if (reset) {
    tracker.resetAt = Date.now();
    tracker.lastText = "";
    hideSpeechBubble(true);
  }
  resolveAndWatchTranscript(sessionId);
  queueSpeechRefresh(sessionId, reset ? 120 : 20);
}

function handleSpeechEvent(sessionId, event) {
  if (!sessionId) return;

  if (event === "UserPromptSubmit") {
    activateSpeechTracking(sessionId, { reset: true });
    return;
  }

  if (event === "Stop" || event === "PostCompact") {
    activateSpeechTracking(sessionId);
    queueSpeechRefresh(sessionId, 20);
    scheduleSpeechHide(SPEECH_END_HIDE_DELAY_MS);
    return;
  }

  if (event === "SessionEnd") {
    cleanupSpeechTracker(sessionId);
    if (speechCurrentSessionId === sessionId) {
      speechCurrentSessionId = null;
      clearSpeechHideTimer();
      hideSpeechBubble(true);
    }
    return;
  ...
  if (speechCurrentSessionId === sessionId) {
    queueSpeechRefresh(sessionId, 20);
    scheduleSpeechHide(SPEECH_HIDE_DELAY_MS);
  }
}
...
function disableDoNotDisturb() {
  ...
  if (speechCurrentText) showSpeechBubble(speechCurrentText);
  if (miniMode) {
...
function startHttpServer() {
  ...
  httpServer.listen(CLAWD_PORT, "127.0.0.1", () => {
    console.log(`Clawd state server listening on 127.0.0.1:${CLAWD_PORT}`);
  });

  httpServer.on("error", (err) => {
    if (err.code === "EADDRINUSE") {
      console.warn(`Port ${CLAWD_PORT} is in use — running in idle-only mode (no state sync)`);
    } else {
...
function startTopmostWatchdog() {
  ...
  topmostWatchdog = setInterval(() => {
    ...
    if (speechBubble && !speechBubble.isDestroyed() && speechBubble.isVisible()) speechBubble.setAlwaysOnTop(true, WIN_TOPMOST_LEVEL);
  }, TOPMOST_WATCHDOG_MS);
}
...
// ── Screenshot & Ask Yuanbao ──
const SCREENASK_SCRIPT = path.join(__dirname, "..", "screen_ask.py");

function screenshotAndAsk() {
  const proc = spawn("python", [SCREENASK_SCRIPT, "--quick", "-q", "请描述这张截图里有什么"], {
      cwd: path.dirname(SCREENASK_SCRIPT),
    ...
    proc.on("close", (code) => {
      ...
      if (code === 0) {
        showSpeechBubble(t("screenshotSent"));
      } else {
        const errMsg = output.trim().split("\n").pop() || `错误码 ${code}`;
        showSpeechBubble(`截图发送失败: ${errMsg}`);
      }
    ...
    proc.on("error", (err) => {
      ...
      showSpeechBubble(`启动失败: ${err.message}`);
...
if (!gotTheLock) {
  ...
  app.on("before-quit", () => {
    ...
    clearSpeechHideTimer();
    for (const sessionId of [...speechTrackers.keys()]) cleanupSpeechTracker(sessionId);
    if (speechBubble && !speechBubble.isDestroyed()) speechBubble.destroy();
